...
def _parse_label_specs(raw_specs: list[str]) -> dict[str, list[int]]:
    ...
    for spec in raw_specs:
        if "=" not in spec:
            raise SystemExit(
                f"Invalid --label value: {spec!r}. Expected label=page,page"
            )
...
def _resolve_labels(
    set_num: str, pdf_name: str, raw_specs: list[str]
) -> dict[str, list[int]]:
    if raw_specs:
...
def _resolve_source_image(
    debug_pdf_dir: Path, page_number: int, payload: dict[str, Any]
) -> Path | None:
...
def _crop_box(
    image_width: int, image_height: int, crop_name: str
...
def _export_crops(
    source_image: Path, destination_dir: Path, stem: str
    ...
    for label, page_numbers in labels.items():
        ...
        for page_number in page_numbers:
            ...
            if source_image is None:
                raise SystemExit(
                    f"No source image found for page {page_number} in {debug_pdf_dir}"
                )
            ...
            crop_paths: dict[str, Path | None] = {
                crop_name: None for crop_name in CROP_NAMES
            }
            crop_note: str | None = None
            if not args.no_crop:
                crop_paths, crop_note = _export_crops(
                    source_image, destination_dir, stem
                )
            ...
            training_payload: dict[str, Any] = {
                ...
                "top_left_crop_path": (
                    crop_paths["top_left"].as_posix()
                    if crop_paths["top_left"]
                    else None
                ),
                "top_right_crop_path": (
                    crop_paths["top_right"].as_posix()
                    if crop_paths["top_right"]
                    else None
                ),
                "top_center_crop_path": (
                    crop_paths["top_center"].as_posix()
                    if crop_paths["top_center"]
                    else None
                ),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export labeled training data from existing debug page renders."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-crop", action="store_true", help="Skip optional top-region crop export."
    )
