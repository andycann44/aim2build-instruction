"""
Bag 4 V1-style per-page completeness gate report.
...
from recover_bag4_v1_callouts import run_v1_callout_recovery


PROJECT_ROOT = ROOT_DIR.parent
PAGES_DIR = PROJECT_ROOT / "debug" / "70618" / "70618_01" / "pages"
CROP_CACHE_PATH = PROJECT_ROOT / "debug" / "crop_cache" / "70618_bag4.json"
TRAINING_LABELS_PATH = PROJECT_ROOT / "debug" / "training_labels" / "70618_bag4.json"
...
OUT_DIR = ROOT_DIR / "debug" / "bag4_page_review"
...
V1_RECOVERY_DIR = ROOT_DIR / "debug" / "bag4_v1_callout_recovery"
...
PERMANENT_LABELS: Dict[Tuple[int, int], Dict[str, Any]] = {
    (58, 4): {
        "human_label": "FALSE_STEP",
        "permanent": True,
        "labeled_by": "page_review",
        "notes": "Page 58: number 4 is Bag 4 artwork label, not a build step",
        "evidence_png": "debug/bag4_page_review/page_058/page_annotated.png",
    ...
    (67, 103): {
        ...
        "permanent": True,
        "labeled_by": "page_review",
        "notes": "Page 67: real build step 103; model highlights only, no parts callout box",
        "evidence_png": "debug/bag4_page_review/page_067/page_annotated.png",
    },
    (66, 102): {
        "human_label": "EMPTY_STEP",
        "permanent": True,
        ...
        "notes": (
            "Page 66 step 102: V1 recovery rejected — detected contour is page separator geometry, "
            "not a parts callout; crop spans page region with no usable parts; not for training."
        ),
        "evidence_png": "debug/bag4_step102_revalidation/v1_recovery/step_102/side_by_side.png",
        "callout_visible": False,
    },
}
...
AUDIT_STEP_CLASSIFICATIONS: Dict[Tuple[int, int], Dict[str, Any]] = {
    (61, 84): {
        "human_label": "REAL_CALLOUT",
        "labeled_by": "page_audit",
        "notes": "Page 61: step 84 anchor + callout box (1x black bracket); V1 border detect conf 0.9995",
        "evidence_png": "debug/bag4_v1_callout_recovery/step_084/side_by_side.png",
        "callout_visible": True,
    },
    (62, 86): {
        "human_label": "REAL_CALLOUT",
        "labeled_by": "page_audit",
        "notes": "Page 62: step 86 anchor + callout box (2x red plate, 2x brown slope); V1 conf 0.9994",
        "evidence_png": "debug/bag4_v1_callout_recovery/step_086/side_by_side.png",
        "callout_visible": True,
    },
    (64, 94): {
        "human_label": "EMPTY_STEP",
        "labeled_by": "page_audit",
        "notes": "Page 64: step 94 anchor present; sub-assembly attach only, no parts callout box",
        "evidence_png": "debug/bag4_page_review/page_064/page_annotated.png",
        "callout_visible": False,
    },
    (75, 121): {
        "human_label": "EMPTY_STEP",
        "labeled_by": "page_audit",
        "notes": "Page 75: step 121 visible; stern arrow only, no parts callout box (no OCR anchor)",
        "evidence_png": "debug/bag4_page_review/page_075/page_full.png",
        "callout_visible": False,
    },
}

AUDIT_STEPS = sorted(
    {step for _page, step in AUDIT_STEP_CLASSIFICATIONS} | {step for _page, step in PERMANENT_LABELS}
)
...
def _resolved_label_record(page: int, step: int) -> Optional[Dict[str, Any]]:
    ...
    step = int(step)
    for (_page, labeled_step), record in PERMANENT_LABELS.items():
        if int(labeled_step) == step:
            return dict(record)
    return None
...
def _supplemental_sequence_gap_reviews(
    ...
    last = known[-1]
    if int(last["step"]) == 131 and 132 not in known_steps:
        supplemental.append(
            {
                "step": 132,
                "page": 80,
                ...
                "next_known": {
                    ...
                    "page": 82,
                    "crop_id": None,
                    "note": "bag 5 step_map anchor; outside bag 4 crop_cache",
                },
                "inference_note": (
                    "bag 4 crop_cache ends at step 131 (page 79, p79_s131_c1); page 80 visibly "
                    "contains step 132. V2 step_map OCR emits misread 1; PSM-6 page OCR reads 132. "
                    "Next sequence anchor step 133 is on page 82 (bag 5)."
                ),
                "sequence_gap_inferred": True,
                "bag_boundary_gap": True,
            }
        )
    ...
    entries.append(
        {
            ...
            "note": "completeness gate item; awaiting human_label in bag4_page_review/human_labels.json",
        }
    )
...
<head>
  ...
  <title>Bag 4 Completeness Gate — pages {page_start}-{page_end}</title>
  <style>
    :root {{
      color-scheme: light;
...
<body>
  <h1>Bag 4 Per-Page Completeness Gate</h1>
  <div class="top-meta">
...
def build_report() -> Path:
    ...
    ocr_noise_audit = audit_bag_ocr_noise(BAG)
    ocr_noise_keys = {(int(row["page"]), int(row["ocr_value"])) for row in ocr_noise_audit.get("rows", [])}
    _write_json(OCR_NOISE_DIAGNOSTICS_PATH, ocr_noise_audit)

    misread_audit = audit_bag_misread_steps(BAG, ocr_noise_keys=ocr_noise_keys)
    substep_audit = audit_bag_substep_numbers(BAG, ocr_noise_keys=ocr_noise_keys)
    ...
    human_labels = {
        "name": "bag4_page_review_human_labels",
        "set_num": SET_NUM,
        "bag": BAG,
        "page_start": page_start,
        "page_end": page_end,
